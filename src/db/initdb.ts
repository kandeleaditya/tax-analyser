...
try {
   db.prepare(createTableStatement).run();
...
`).run();
 */
/* id int unsigned AUTO_INCREMENT PRIMARY KEY,  -- Auto-incrementing integer for ID
        username varchar(255) NOT NULL UNIQUE,  -- Username should be unique
        name varchar(255) NOT NULL,
        email varchar(255) NOT NULL UNIQUE,  -- Email should also be unique
        phone varchar(20),
        status enum('active', 'inactive') NOT NULL DEFAULT 'active',  -- Status with default value
        plan_end_date date,
        DBPrefix varchar(20),
